  ...
  const createWeightBuf = (device, size, data) => {
    ...
    const buf = device.createBuffer({ size: size, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST });
    data.bytes = buf;
    return buf;
  };
...
@compute @workgroup_size(256) fn main(@builtin(workgroup_id) gindex: vec3<u32>,@builtin(local_invocation_id) lindex: vec3<u32>) {
  ...
  if (((bool(lidx0))!=true)) {
   ...
   await new Promise(resolve => setTimeout(resolve, 0));
   progress(74, 100, 'Launching WebGPU model:');
